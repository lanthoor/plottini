...
class TestConfigurationRoundTrip:
    ...
    def test_full_config_roundtrip(self, tmp_path: Path) -> None:
        ...
        original = GrapherConfig(
            files=[
                FileConfig(path=Path("data.tsv"), has_header=True),
            ],
            derived_columns=[
        ...
        assert loaded.files[0].has_header == original.files[0].has_header

        assert len(loaded.derived_columns) == 1
        assert loaded.derived_columns[0].name == "velocity"
        assert loaded.derived_columns[0].expression == "distance / time"

        assert len(loaded.filters) == 1
        assert loaded.filters[0].column == "time"
        assert loaded.filters[0].min == 0.0
        assert loaded.filters[0].max == 100.0

        assert len(loaded.series) == 1
        assert loaded.series[0].x == "time"
        assert loaded.series[0].y == "velocity"
        assert loaded.series[0].label == "Velocity"
        assert loaded.series[0].color == "#0072B2"
        assert loaded.series[0].line_style == "--"
        assert loaded.series[0].marker == "o"

        assert loaded.plot.type == "line"
        assert loaded.plot.title == "Velocity Analysis"
        assert loaded.plot.figure_width == 12.0
        assert loaded.plot.figure_height == 8.0

        assert loaded.export.format == "pdf"
        assert loaded.export.dpi == 600
...
class TestConfigRoundtripExtended:
    """Extended configuration roundtrip tests."""

    def test_config_with_all_plot_options(self, tmp_path: Path) -> None:
        """Test config roundtrip with all plot options set."""
        original = GrapherConfig(
            files=[FileConfig(path=Path("data.tsv"), has_header=True)],
            series=[
                SeriesConfigSchema(
                    x="x",
                    y="y",
        ...
        assert loaded.plot.y2_label == "Secondary Y"
        assert loaded.plot.figure_width == 14.0
        assert loaded.plot.show_grid is False
        assert loaded.export.transparent is True
        assert loaded.series[0].line_width == 2.5
        assert loaded.series[0].use_secondary_y is True

    def test_config_minimal(self, tmp_path: Path) -> None:
        ...
        assert loaded.plot.type == "line"
        assert loaded.export.dpi == 300

    def test_config_with_multiple_files(self, tmp_path: Path) -> None:
        """Test config roundtrip with multiple data files."""
        original = GrapherConfig(
            files=[
                FileConfig(path=Path("data1.tsv"), has_header=True, delimiter="\t"),
                FileConfig(path=Path("data2.tsv"), has_header=False, delimiter=","),
                FileConfig(path=Path("data3.tsv"), comment_chars=["#", "!"]),
            ],
            series=[
                SeriesConfigSchema(x="x", y="y", source_file_index=0),
                SeriesConfigSchema(x="x", y="y", source_file_index=1),
                SeriesConfigSchema(x="x", y="y", source_file_index=2),
            ],
        )

        config_path = tmp_path / "multi_file.toml"
        save_config(original, config_path)
        loaded = load_config(config_path)

        assert len(loaded.files) == 3
        assert loaded.files[0].delimiter == "\t"
        assert loaded.files[1].delimiter == ","
        assert loaded.files[1].has_header is False
        assert loaded.files[2].comment_chars == ["#", "!"]

    def test_config_with_all_filter_types(self, tmp_path: Path) -> None:
        """Test config with various filter configurations."""
        original = GrapherConfig(
            files=[FileConfig(path=Path("data.tsv"))],
            filters=[
                FilterConfig(column="x", min=0.0),
                FilterConfig(column="y", max=100.0),
                FilterConfig(column="z", min=10.0, max=50.0),
            ],
            series=[SeriesConfigSchema(x="x", y="y")],
        )

        config_path = tmp_path / "filters.toml"
        save_config(original, config_path)
        loaded = load_config(config_path)

        assert len(loaded.filters) == 3
        assert loaded.filters[0].min == 0.0
        assert loaded.filters[0].max is None
        assert loaded.filters[1].min is None
        assert loaded.filters[1].max == 100.0
        assert loaded.filters[2].min == 10.0
        assert loaded.filters[2].max == 50.0

    def test_config_with_alignment(self, tmp_path: Path) -> None:
        """Test config roundtrip with alignment configuration."""
        from plottini.config.schema import AlignmentConfig

        original = GrapherConfig(
            files=[FileConfig(path=Path("data.tsv"))],
            alignment=AlignmentConfig(enabled=True, column="time"),
            series=[SeriesConfigSchema(x="time", y="value")],
        )

        config_path = tmp_path / "alignment.toml"
        save_config(original, config_path)
        loaded = load_config(config_path)

        assert loaded.alignment is not None
        assert loaded.alignment.enabled is True
        assert loaded.alignment.column == "time"


class TestEndToEndWorkflows:
    """End-to-end workflow tests from config to rendered output."""

    def test_complete_workflow_with_config_file(self, tmp_path: Path) -> None:
        """Test complete workflow: create data, create config, render."""
        # 1. Create data file
        data_path = tmp_path / "experiment.tsv"
        data_path.write_text(
            "time\tmeasurement\n0.0\t10.5\n0.5\t12.3\n1.0\t15.7\n1.5\t14.2\n2.0\t18.9\n"
        )
        ...
        config = GrapherConfig(
            files=[FileConfig(path=data_path, has_header=True)],
            series=[
                SeriesConfigSchema(
                    x="time",
                    y="measurement",
        ...
        parser = TSVParser(ParserConfig(has_header=True))
        df = parser.parse(loaded_config.files[0].path)
        ...
        series = [
            ...
            for s in loaded_config.series
        ]
        plot_config = PlotConfig(
            chart_type=ChartType.LINE,
            title=loaded_config.plot.title,
            x_label=loaded_config.plot.x_label,
            y_label=loaded_config.plot.y_label,
        )
        plotter = Plotter(plot_config)
        fig = plotter.create_figure(df, series)
        ...
        # 6. Export
        exporter = Exporter()
        output_path = tmp_path / "experiment.png"
        exporter.export(fig, output_path, ExportConfig(format=ExportFormat.PNG))
